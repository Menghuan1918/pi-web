...
export async function GET(req: Request) {
  ...
  try {
    ...
    return compressedJson(req, await loadSkillsWithInstallInfo(cwd));
  } catch (e) {
    return compressedJson(req, { error: String(e) }, { status: 500 });
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    if (!filePath) return compressedJson(req, { error: "filePath required" }, { status: 400 });
    if (!existsSync(filePath)) return compressedJson(req, { error: "file not found" }, { status: 404 });
