...
export async function POST(req: Request) {
  try {
    ...
    if (!query?.trim()) return compressedJson(req, { error: "query required" }, { status: 400 });
    const limit = parseLimit(rawLimit);

    try {
      const results = await searchSkillsApi(query.trim(), limit);
      return compressedJson(req, { results });
    } catch {
    ...
    if (results.length > 0) return compressedJson(req, { results });
    return compressedJson(req, { error: err.message ?? String(e) }, { status: 500 });
  }
}
